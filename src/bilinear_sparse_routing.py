...
class BilinearSparseRouting(nn.Module):
    def __init__(self, next_bucket_size, 
                ...
                matrix_pose, layer_type, seed, kernel_size=None,
                temperature = 0.75,
    ...
    def forward(self, current_pose, h_out=1, w_out=1, next_pose=None):
        ...
        # if self.layer_type =='FC':
            # print("Current pose ", current_pose.shape)
        ...
        if next_pose is None:
            ...
            # if self.layer_type =='FC':
                # print("Current pose1 ", current_pose.shape)
            if self.matrix_pose:
            ...
            # if self.layer_type =='FC':
                # print("Current pose2 ", current_pose.shape)
            if self.matrix_pose:
            ...
            # w_current = (1,288,4,4)
            w_current = w_current.unsqueeze(0)  
            w_next = w_next.unsqueeze(0) #(1,288,16,16)
            ...
            if self.matrix_pose:
                ...
                b_next_pose_candidates = b_next_pose_candidates.unsqueeze(3) #(49b, bucket, bucket_size, 1,16)
            
            # Multiplied with N_{j} to get final pose
            # w_next: (49b,m,4,4); b_next_pose_candidates: (49b, bucket, bucket_size, 4, 4)
            # print(b_next_pose_candidates.shape, w_next.shape)
...
# Capsules with bilinear sparse routing but random gaussian initialisations
class BilinearRandomInitRouting(nn.Module):
    def __init__(self, next_bucket_size, 
                in_n_capsules, in_d_capsules, out_n_capsules, out_d_capsules, 
                matrix_pose, layer_type, seed,kernel_size=None,
                temperature = 0.75,
    ...
    def forward(self, current_pose, h_out=1, w_out=1, next_pose=None):
        ...
        if next_pose is None:
            ...
            # next_pose = torch.zeros(batch_size,self.out_n_capsules,h_out, w_out, self.pose_dim).type_as(current_pose).to(current_pose)
...
# Bilinear without sinkhorn with multi head attention like transformers
class MultiHeadBilinearRouting(nn.Module):
    def __init__(self, next_bucket_size, 
                in_n_capsules, in_d_capsules, out_n_capsules, out_d_capsules, 
                matrix_pose, layer_type, seed,kernel_size=None, num_heads = 4,
                temperature = 0.75,
        non_permutative = True, sinkhorn_iter = 7, n_sortcut = 0, dropout = 0., current_bucket_size = None,
        use_simple_sort_net = False):
        super().__init__()
        ...
        assert (self.in_n_capsules % self.num_heads == 0), "Number of heads should divide capsule dimension"
        self.new_in_n_caps = (int)(self.in_n_capsules / self.num_heads)
        ...
        if matrix_pose:
            ...
            # w_current =(heads, 3,3,32,4,4)
            self.w_current = nn.Parameter(0.02*torch.randn(self.num_heads, kernel_size, kernel_size,
                                                     self.new_in_n_caps, self.matrix_pose_dim, self.matrix_pose_dim))
            self.w_next = nn.Parameter(0.02*torch.randn(self.num_heads,
                                                     out_n_capsules, self.matrix_pose_dim, self.matrix_pose_dim))
        else:
            self.w_current = nn.Parameter(0.02*torch.randn(self.num_heads,kernel_size, kernel_size,
                                                     self.new_in_n_caps, self.pose_dim, self.pose_dim))
            self.w_next = nn.Parameter(0.02*torch.randn(self.num_heads,
                                                     out_n_capsules, self.pose_dim, self.pose_dim))
    ...
    def forward(self, current_pose, h_out=1, w_out=1, next_pose=None):
        ...
        if next_pose is None:
            ...
            w_next = self.w_next
            if self.matrix_pose:
                #w_current =(head,3,3,32,4,4) --> (head,3*3*32, 4, 4) == (4,3*3*8,4,4)
                w_current = w_current.view(self.num_heads, self.kernel_size*self.kernel_size*self.new_in_n_caps, self.matrix_pose_dim, self.matrix_pose_dim)
            else:
                w_current = w_current.view(self.num_heads, self.kernel_size*self.kernel_size*self.new_in_n_caps, self.pose_dim, self.pose_dim)
            
            #
            # W_current is C_{L} and w_next is N_{L}
            w_current = w_current.unsqueeze(0)  # (1,3,3*3*8,4,4)
            w_next = w_next.unsqueeze(0)

            # (49b,4,3*3*8,16)
            current_pose = current_pose.reshape(batch_size*h_out*w_out, self.num_heads, self.kernel_size*self.kernel_size*self.new_in_n_caps, self.pose_dim)#view error
            
            if self.matrix_pose:
                # (b*7*7,4,3*3*8,4,4) = (49b,4,72,4, 4)
                # print(current_pose.shape)
                current_pose = current_pose.reshape( batch_size*h_out*w_out, self.num_heads, self.kernel_size*self.kernel_size*self.new_in_n_caps, self.matrix_pose_dim, self.matrix_pose_dim)#replace the 2 reshapes
            else:
                current_pose = current_pose.unsqueeze(2)
            
            # Multiplying p{L} by C_{L} to change to c_{L}
            # Current pose: (49b,4,72, 4, 4), w_current = (1,4,74,4,4)
            # Same matrix for the entire batch, output  = (49b, 4,72, 4, 4)
            current_pose = torch.matmul(current_pose, w_current) 
            
            if self.matrix_pose:
                # Current_pose = (49b,4,72, 16)
                current_pose = current_pose.reshape(batch_size*h_out*w_out, self.num_heads, self.kernel_size*self.kernel_size*self.new_in_n_caps, self.pose_dim)
            else:
                current_pose = current_pose.squeeze(2)
            
            # R_{i,j} = (49b, 4, m, 72)
            dots=(torch.ones(batch_size*h_out*w_out, self.num_heads, self.out_n_capsules, self.kernel_size*self.kernel_size*self.new_in_n_caps)* (pose_dim ** -0.5)).type_as(current_pose).to(current_pose)
            dots = dots.softmax(dim=-2)
            next_pose_candidates = current_pose  
            # Multiplies r_{i,j} with c_{L} ( no sorting in the 1st iteration) to give X. Still have to
            # multiply with N_{L} 
            # next pose: (49b,4,32,16) 
            next_pose_candidates = torch.einsum('bkij,bkje->bkie', dots, next_pose_candidates)
            ...
            if self.matrix_pose:
                ...
                next_pose_candidates = next_pose_candidates.view(next_pose_candidates.shape[0], next_pose_candidates.shape[1], next_pose_candidates.shape[2], self.matrix_pose_dim, self.matrix_pose_dim)
            else:
                next_pose_candidates = next_pose_candidates.unsqueeze(2)
            ...
            # Reshape: (b, 7, 7, k*m, 16)
            next_pose_candidates = next_pose_candidates.view(batch_size, h_out, w_out, self.out_n_capsules * self.num_heads,  self.pose_dim)
            ...
            current_pose = current_pose.reshape(batch_size*h_out*w_out, self.num_heads, self.kernel_size*self.kernel_size*self.new_in_n_caps, self.pose_dim)            
            if self.matrix_pose:
                # Current_pose = (49b, 4, 72, 4, 4)
                current_pose = current_pose.reshape(batch_size*h_out*w_out, self.num_heads, self.kernel_size*self.kernel_size*self.new_in_n_caps, self.matrix_pose_dim, self.matrix_pose_dim)#replace the 2 reshapes
            else:
                current_pose = current_pose.unsqueeze(2)
            
            # Tranformed currentlayer capsules to c_{L}
            # Multiply (49b, 4, 72, 4, 4) with (1,4,72,4,4) --> (49b,4,72, 4, 4)
            current_pose = torch.matmul(current_pose, w_current)
            ...
            # Tranform next pose using N_{L}: w_next = (49b,4,m,4,4) * (1,4,m,4,4)
            next_pose = torch.matmul(w_next, next_pose)
            

            if self.matrix_pose:
                # next_pose = (49b,4,m,16)
            ...
            # Yet to multiply with N_{L} (next_w) # (49b,4,m,pose)
            next_pose_candidates = torch.einsum('bkji,bkie->bkje', dots, next_pose_candidates)
            
            if self.matrix_pose:
                # next pose: 49b,4,m,16 --> 49b,4,m,4,4
                next_pose_candidates = next_pose_candidates.view(next_pose_candidates.shape[0], next_pose_candidates.shape[1], next_pose_candidates.shape[2], self.matrix_pose_dim, self.matrix_pose_dim)
            else:
                next_pose_candidates = next_pose_candidates.unsqueeze(3)
            
            # Multiplied with N_{j} to get final pose
            # w_next: (1,4,m,4,4); next_pose_candidates: (49b,4,m,4, 4)
            next_pose_candidates = torch.matmul(w_next, next_pose_candidates)
            
            # next_pose_candidates = (b,7,7,k*m,16)
            next_pose_candidates = next_pose_candidates.view(batch_size, h_out, w_out,  self.num_heads* self.out_n_capsules,  self.pose_dim)
...
# #Capsules WITHOUT Sinkhorn checking "Vector poses"
# class BilinearVectorRouting(nn.Module):
#     def __init__(self, next_bucket_size, 
#                 in_n_capsules, in_d_capsules, out_n_capsules, out_d_capsules, 
#                 matrix_pose, layer_type,seed, kernel_size=None,
#                 temperature = 0.75,
...
class DynamicBilinearRouting(nn.Module):
    ...
    def forward(self, current_pose, dots, h_out=1, w_out=1):
        ...
        #
        # print("Current img ", current_pose.shape)
        # W_current is C_{L} and w_next is N_{L}
        w_current = w_current.unsqueeze(0)  
        w_next = w_next.unsqueeze(0)
