...
class LocalLinformerProjection(nn.Module):
    def __init__(self, 
                ...
                matrix_pose, layer_type, input_img_size, output_img_size, hidden_dim=None, kernel_size=None, multi_transforms=False, kernel_transformation = False, parameter_sharing='headwise',
                dropout = 0.):
        super().__init__()
...
class MultiHeadLocalLinformerProjection(nn.Module):
    def __init__(self, 
                in_n_capsules, in_d_capsules, out_n_capsules, out_d_capsules, 
                matrix_pose, layer_type, input_img_size, output_img_size, hidden_dim=None, kernel_size=None,num_heads=8, multi_transforms=False, kernel_transformation = False, parameter_sharing='headwise',
                dropout = 0.):
        super().__init__()
    ...
    def forward(self, current_pose, h_out=1, w_out=1, next_pose=None):
        ...
        if next_pose is None:
            ...
            ######### Complexity reduction step instead of einsum
            next_pose_candidates=torch.sum(next_pose_candidates, dim=1) * value #49b,1,16
            next_pose_candidates = next_pose_candidates.unsqueeze(1).unsqueeze(1) # 49b,1,1,16
            ...
            # Multiplied with N_{j} to get final pose
            # w_next: (49b,k,m,4,4); b_next_pose_candidates: (49b,k,m , 4, 4)
            next_pose_candidates = torch.matmul(next_pose_candidates, w_next)
