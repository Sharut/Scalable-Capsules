...
class LocalLinformerProjection(nn.Module):
    def __init__(self, 
                ...
                matrix_pose, layer_type, input_img_size, output_img_size, seed, hidden_dim=None, kernel_size=None, multi_transforms=False, kernel_transformation = False, parameter_sharing='headwise',
                dropout = 0.):
        super().__init__()
        seed_torch(seed)
        ...
        if parameter_sharing == "headwise":
            # print("Hello")
            if not self.multi_transforms:
                self.E_proj = nn.Parameter(0.02*torch.randn(kernel_size, kernel_size,
                                                         in_n_capsules, hidden_dim))
                print("You are using Local Linformer with single matrix transformations")
            else:
                self.num_transform_matrices = 4
                self.reduced_dim = (int)(self.hidden_dim/self.num_transform_matrices)
                print("Using "+str(self.num_transform_matrices)+" different tranformation matrices each to change " + str(kernel_size*kernel_size*in_n_capsules)+" dims to " + str(self.reduced_dim))
                self.E_proj = nn.Parameter(0.02*torch.randn(self.num_transform_matrices, kernel_size, kernel_size,
                                                         in_n_capsules, self.reduced_dim))

                print(self.E_proj.shape)
                print("You are using Local Linformer with multiple transformations")
        ...
        if next_pose is None:
            ...
            if self.kernel_transformation == False:
                if not self.multi_transforms:
                    ...
                    current_pose = current_pose.permute(2,0,1).unsqueeze(1) # (16,1,49b,288)
                    E_proj = self.E_proj.view(self.num_transform_matrices,self.kernel_size*self.kernel_size*self.in_n_capsules, self.reduced_dim) # (4,288,hidden_dim/4) == (4,288,32)    ( if hidden dim==128)      

                    current_pose = torch.matmul(current_pose, E_proj) # (16,4,49b,hidden_dim/4)
                    current_pose = current_pose.permute(0,2,1,3) # (16,49b,4,hidden_dim/4)
                    current_pose = current_pose.reshape(current_pose.shape[0], current_pose.shape[1], -1) # (16,49b,hidden_dim)
                    current_pose = current_pose.permute(1,2,0) # (49b, hidden_Dim, 16)
...
########## Linformer projection on each kernel
class MultiHeadLocalLinformerProjection(nn.Module):
    def __init__(self, 
                in_n_capsules, in_d_capsules, out_n_capsules, out_d_capsules, 
                matrix_pose, layer_type, input_img_size, output_img_size, seed, hidden_dim=None, kernel_size=None,num_heads=8, multi_transforms=False, kernel_transformation = False, parameter_sharing='headwise',
                dropout = 0.):
        super().__init__()
        seed_torch(seed)
        self.in_d_capsules = in_d_capsules
        ...
        if matrix_pose:
            ...
            self.w_current = nn.Parameter(0.02*torch.randn(kernel_size, kernel_size,
                                                     in_n_capsules, self.matrix_pose_dim, self.matrix_pose_dim))
            self.w_next = nn.Parameter(0.02*torch.randn(self.num_heads,
                                                     out_n_capsules, self.matrix_pose_dim, self.matrix_pose_dim))
        else:
            self.w_current = nn.Parameter(0.02*torch.randn(kernel_size, kernel_size,
                                                     in_n_capsules, self.pose_dim, self.pose_dim))
        ...
        if parameter_sharing == "headwise":
            # print("Hello")
            if not self.multi_transforms:
                self.E_proj = nn.Parameter(0.02*torch.randn(self.num_heads, kernel_size, kernel_size,
                                                         in_n_capsules, hidden_dim))
                print("You are using Local Linformer with single matrix transformations")
    ...
    def forward(self, current_pose, h_out=1, w_out=1, next_pose=None):
        ...
        if next_pose is None:
            ...
            if self.kernel_transformation == False:
                if not self.multi_transforms:
                    current_pose = current_pose.permute(2,0,1).unsqueeze(1) # (16,1,49b,288)
                    E_proj = self.E_proj.view(self.num_heads, self.kernel_size, self.kernel_size, self.in_n_capsules, self.hidden_dim) # (k,288, hidden_dim)            
                    E_proj = E_proj.permute(0,4,1,2,3) # k,hidden,h,w,n
                    E_proj = E_proj.reshape(self.num_heads, self.hidden_dim, -1) # k,hidden,288
                    E_proj = E_proj.permute(0,2,1) # k,288,hidden
                    current_pose = torch.matmul(current_pose, E_proj) # (16,k,49b,hidden_dim)
                    current_pose = current_pose.permute(2,1,3,0) # (49b,k,hidden_Dim, 16)
            ...
            ######### Complexity reduction step instead of einsum
            next_pose_candidates=torch.sum(next_pose_candidates, dim=2) * value # 49b,k,16
            next_pose_candidates = next_pose_candidates.unsqueeze(2) # 49b,k,1,16
            next_pose_candidates = next_pose_candidates.expand(next_pose_candidates.shape[0], self.num_heads, self.out_n_capsules, next_pose_candidates.shape[3]) #49b,k,m,16


            if self.matrix_pose:
                # Correct shapes: (49b, k, m, 4, 4)
                next_pose_candidates = next_pose_candidates.reshape(next_pose_candidates.shape[0], next_pose_candidates.shape[1], next_pose_candidates.shape[2], self.matrix_pose_dim, self.matrix_pose_dim)
            else:
                next_pose_candidates = next_pose_candidates.unsqueeze(2)
            
            # Found final pose of next layer by multiplying X with N_{L}
            # Multiply (49b, k,m, 4, 4) with (1,k, m, 4, 4) == (49b,k, m , 4, 4)
            next_pose_candidates = torch.matmul(next_pose_candidates, w_next)

            # Reshape: (b, 7, 7, m, 16)
            next_pose_candidates = next_pose_candidates.reshape(batch_size * h_out * w_out, self.num_heads, self.out_n_capsules,  self.pose_dim) #49b,k,m,16
            next_pose_candidates = next_pose_candidates.permute(0,3,1,2) # 49b,16,k,m
            next_pose_candidates = next_pose_candidates.reshape(batch_size * h_out * w_out, self.pose_dim, self.num_heads * self.out_n_capsules) # 49b,16,k*m
            next_pose_candidates = next_pose_candidates.permute(0,2,1) # 49b,k*m,16
            next_pose_candidates = next_pose_candidates.reshape(batch_size,h_out,w_out, self.num_heads * self.out_n_capsules, self.pose_dim)
            ...
            if self.kernel_transformation == False:
                if not self.multi_transforms:
                    current_pose = current_pose.permute(2,0,1).unsqueeze(1) # (16,1,49b,288)
                    E_proj = self.E_proj.view(self.num_heads, self.kernel_size, self.kernel_size, self.in_n_capsules, self.hidden_dim) # (k,288, hidden_dim)            
                    E_proj = E_proj.permute(0,4,1,2,3)
                    E_proj = E_proj.reshape(self.num_heads, self.hidden_dim, -1) # k,hidden,288
                    E_proj = E_proj.permute(0,2,1) # k,288,hidden
                    current_pose = torch.matmul(current_pose, E_proj) # (16,k,49b,hidden_dim)
                    current_pose = current_pose.permute(2,1,3,0) # (49b,k,hidden_Dim, 16)
                ...
                if self.layer_type == 'conv':
                    current_pose = current_pose.reshape(batch_size*h_out*w_out, self.in_n_capsules, self.kernel_size, self.kernel_size, self.pose_dim)
                    current_pose = current_pose.permute(4,0,1,2,3) # pose,49b,32,3,3
                    current_pose = current_pose.reshape(self.pose_dim,batch_size*h_out*w_out, self.in_n_capsules, -1)
                    current_pose = current_pose.unsqueeze(3).unsqueeze(2) # pose,49b,1,32,1,9
                    E_proj = self.E_proj.permute(0,4,3,1,2) # k,hidden_dim,n,k_h,k_w
                    E_proj = E_proj.view(self.num_heads, self.hidden_dim, self.in_n_capsules, self.kernel_size*self.kernel_size) 
                    E_proj = E_proj.permute(0,2,3,1) # (k,32,9,hidden)        
                    # print(current_pose.shape, E_proj.shape)
                    current_pose = torch.matmul(current_pose, E_proj).squeeze(4) # (16,49b,k,32,1,hidden_dim)
                    # print(current_pose.shape)
                    current_pose = current_pose.view(current_pose.shape[0], current_pose.shape[1], current_pose.shape[2], -1) # (16,49b,k, 32*hidden_dim)
                    current_pose = current_pose.permute(1,2,3,0) # (49b, k, hidden_Dim, 16)
                    
                else:
                    current_pose = current_pose.permute(2,0,1).unsqueeze(1) # (16,1,b,800)
                    E_proj = self.E_proj.permute(0,4,3,1,2) # k,hidden_dim, n, k_h,k_w
                    E_proj = self.E_proj.view(self.num_heads, self.hidden_dim, self.kernel_size*self.kernel_size*self.in_n_capsules) # (k,hidden_dim, 288)            
                    E_proj = E_proj.permute(0,2,1) #k,800,512
                    current_pose = torch.matmul(current_pose, E_proj) # (16,k,b,hidden_dim)
                    current_pose = current_pose.permute(2,1,3,0) # (b,k,hidden_Dim, 16)

                                

            ###################### Reverse engineer to get correct shapes
            # next pose of b,k*m,h,w,pose
            if self.layer_type =='conv':
                next_pose = next_pose.permute([0,2,3,1,4]) # b,h,w,k*m,pose
            else:
                next_pose = next_pose.unsqueeze(1) # (b,1,k*m,16) --> (b,1,1,k*m,16)

            # next_pose_candidates = next_pose_candidates.reshape(batch_size,h_out,w_out, self.num_heads * self.out_n_capsules, self.pose_dim)
            next_pose = next_pose.reshape(-1, self.num_heads * self.out_n_capsules, self.pose_dim) # 49b,k*m,pose
            next_pose = next_pose.reshape(batch_size*h_out*w_out, self.num_heads, self.out_n_capsules,  self.pose_dim)
  

                
            if self.matrix_pose:
                # next_pose = (49b,m,16)  -->  (49b,m,4,4) 
                next_pose = next_pose.reshape(batch_size*h_out*w_out,self.num_heads, self.out_n_capsules,  self.matrix_pose_dim, self.matrix_pose_dim)
            else:
                next_pose = next_pose.unsqueeze(3)
            
            # Tranform next pose using N_{L}: w_next = (49b,k,m,4,4) * (1,k,m,4,4)
            next_pose = torch.matmul(w_next, next_pose)
            

            if self.matrix_pose:
                # next_pose = (49b,k,m,16)
                next_pose = next_pose.view(batch_size*h_out*w_out, self.num_heads, self.out_n_capsules,  self.pose_dim)
            else:
                next_pose = next_pose.squeeze(3)
    
            # Finding scaled alignment scores between updated buckets
            # dots = (49b,k,m,288)
            dots = torch.einsum('bkje,bkie->bkji', next_pose, current_pose) * (pose_dim ** -0.5) 
            ...
            # Yet to multiply with N_{L} (next_w)
            next_pose_candidates = torch.einsum('bkji,bkie->bkje', dots, next_pose_candidates)
            
            if self.matrix_pose:
                # next pose: 49b,k,m,16 --> 49b,k,m,4,4
                next_pose_candidates = next_pose_candidates.reshape(next_pose_candidates.shape[0], next_pose_candidates.shape[1], next_pose_candidates.shape[2], self.matrix_pose_dim, self.matrix_pose_dim)
            else:
                next_pose_candidates = next_pose_candidates.unsqueeze(3)
            
            # Multiplied with N_{j} to get final pose
            # w_next: (1,k,m,4,4); next_pose_candidates: (49b,k,m,4,4)
            next_pose_candidates = torch.matmul(next_pose_candidates, w_next)
...
class MithunSirLocalLinformerProjection(nn.Module):
    def __init__(self, 
                in_n_capsules, in_d_capsules, out_n_capsules, out_d_capsules, 
                matrix_pose, layer_type, input_img_size, output_img_size, seed,hidden_dim=None, kernel_size=None, parameter_sharing='headwise',
                dropout = 0.):
        super().__init__()
        seed_torch(seed)
...
class BilinearGlobalLinformerRandomInitCapsuleFC(nn.Module):
    ...
    def __init__(self, hidden_dim, input_size, in_n_capsules, in_d_capsules, out_n_capsules, out_d_capsules, h_out, child_kernel_size, child_stride, child_padding, parent_kernel_size, parent_stride, parent_padding, parameter_sharing, matrix_pose, dp, seed):
...
class BilinearGlobalLinformerDebugCapsuleFC(nn.Module):
    ...
    def forward(self, current_pose, num_iter, next_pose = None):
        ...
        current_pose = current_pose.reshape(batch_size, -1, self.pose_dim) # (b,256,32,1,a*b) 

        if next_pose is None:
            # dots = (torch.ones(batch_size*h_out*w_out, self.out_n_capsules, new_n_capsules)* (pose_dim ** -0.5)).type_as(k_val).to(k_val)
            # dots = F.softmax(dots, dim=-2)
            value = 1/self.out_n_capsules
            next_pose_candidates = k_val   
            # # print('dots: ', dots.shape, ' k_val ', k_val.shape)
            next_pose_candidates = torch.sum(next_pose_candidates, dim=1) * value   # (b, pose_dim)
            next_pose_candidates = next_pose_candidates.reshape(next_pose_candidates.shape[0], 1,1,1,next_pose_candidates.shape[1])
            next_pose_candidates = next_pose_candidates.expand(next_pose_candidates.shape[0], self.out_n_capsules, h_out, w_out, self.pose_dim) # (b,m,h,w,pose)
            next_pose_candidates = next_pose_candidates.reshape(batch_size*h_out*w_out, self.out_n_capsules, self.pose_dim) #(b.h.w, m, pose)
            ...
            next_pose = next_pose.reshape(h_out, w_out, batch_size, self.out_n_capsules, self.pose_dim)
            k_val = k_val.permute(0,2,1) #(b,pose,num_Caps*hidden_Dim)
            dots = torch.matmul(next_pose, k_val) * (pose_dim ** -0.5) # (h,w,b,m,n)
            # dots = torch.einsum('bje,bie->bji', next_pose, k_val) * (pose_dim ** -0.5)  # Holds iff h_out=1
            dots = dots.softmax(dim=-1) 
            k_val = k_val.permute(0,2,1)
            next_pose_candidates = torch.matmul(dots, k_val)  #(h,w,b,m,pose)
            next_pose_candidates=next_pose_candidates.reshape(-1, self.out_n_capsules, self.pose_dim)
...
# Using convoltions for bilinear tranformatiosn
